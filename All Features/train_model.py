...
def train_kmeans(df, asset, feature_columns, feature_columns_xgb, train_start, train_end, validation_years=5, next_update_date=None, pca=False, n_comp_kmeans=5, pca_xgb=False, n_comp_xgb=5):
    ...
    # Drop features (columns) with any NaNs in training set
    feature_columns = list(df_train[feature_columns].dropna(axis=1).columns)
    feature_columns_xgb = list(df_train[feature_columns_xgb].dropna(axis=1).columns)

    # PCA for KMeans
    if pca:
        pca_model_kmeans = PCA(n_components=n_comp_kmeans)
        X_train_pca = pca_model_kmeans.fit_transform(df_train[feature_columns])
